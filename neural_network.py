...
from load_dataset import to_sparse_tensor
...
class TransConv(nn.Module):
    def __init__(
            ...
            use_weight=True,
            use_act=True,
            trans_method='graph_first_trans'
    ):
        super().__init__()
        ...
        self.trans_method = 'graph_add_trans'
        ...
        self.alpha = alpha
        self.use_act = use_act
        self.degree_weight = nn.Parameter(torch.tensor(0.5))
        self.degree_embedding = nn.Embedding(max_degree_upper_bound + 1, in_channels, padding_idx=0)
...
class HybridBlock(nn.Module):
    ...
    def forward(self, x, edge_index, mask):
        if self.trans_method == 'graph_add_trans':
            x = self.graph_add_trans(x, edge_index)
        elif self.trans_method == 'graph_first_trans':
            x = self.graph_first_trans(x, edge_index)

        x_mask = x[mask]
        pred_community = self.fc(x_mask)

        return x_mask, pred_community
    ...
    def graph_first_trans(self, x, edge_index):
        x2 = self.graph_conv.forward(x, edge_index)
        x1 = self.trans_conv.forward(x2)
        x = self.fc(x1)
        return x
    ...
    def graph_add_trans(self, x, edge_index):
        x1 = self.trans_conv.forward(x, edge_index)
        if self.use_graph:
            x2 = self.graph_conv.forward(x, edge_index)
